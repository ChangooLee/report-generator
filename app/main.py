...
async def generate_dynamic_prompts():
    ...
    try:
        ...
        if not tools_info:
            # 실용적인 기본 프롬프트 사용
            dynamic_prompts = [
                "2025년 1월부터 7월까지 강동구 아파트 매매/전월세 현황을 분석해주세요",
                "2025년 1월부터 6월까지 강남구 상업업무용 부동산 거래를 분석해주세요",
                "2025년 상반기 서울시 오피스텔 매매/임대 시장 트렌드를 분석해주세요",
                "2024년 하반기 대비 2025년 상반기 부동산 거래량 변화를 분석해주세요"
            ]
            return
        ...
        # OpenRouter API 호출
        prompt = f"""다음과 같은 부동산 분석 MCP 도구들이 사용 가능합니다:

{tools_text}

이 도구들을 활용해서 실제로 분석 가능한 구체적인 부동산 분석 프롬프트 4개를 한국어로 생성해주세요.

요구사항:
- 구체적인 지역명 포함 (강남구, 강동구, 서초구 등)
- 명확한 기간 설정 (2025년 1월~7월, 2024년 하반기 등)
- 실제 분석 가능한 부동산 유형 (아파트, 오피스텔, 상업용, 단독주택 등)
- 분석 목적 명시 (매매/전월세 현황, 거래량 변화, 시장 트렌드 등)
...
예시: "2025년 1월부터 6월까지 강남구 아파트 매매가격 변화 추이를 분석해주세요"
